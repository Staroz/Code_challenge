// DATE 08/23/23
export function alternate(s: string): number {
    ...
    for (let index = 0; index < s.length; index++) {
        if (!array.find(x=> x ==s[index])) {
            array.push(s[index])
        }       
    }
    for (let i = 0; i < array.length; i++) {
        for (let j = 1; j < array.length; j++) {
            brr = [...s].filter(x=> x==array[i]|| x==array[j])
            if (brr.join('').match(/([a-zA-Z])\1/g)== null && brr.length>max) {
                    max = brr.length;
            } 
...
YGiven a string, remove characters until the string is made up of any two alternating characters. When you choose a character to remove, all instances of that character must be removed. Determine the longest string possible that contains just two alternating letters.
